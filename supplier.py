...
class supplierClass:
    def __init__(self,root):
        ...
        txt_search=Entry(self.root,textvariable=self.var_searchtxt,font=("goudy old style",15),bg="lightyellow").place(x=780,y=80,width=170)
        ...
        btn_clear=Button(self.root,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607d9b",fg="white",cursor="hand2").place(x=540,y=370,width=110,height=35)
        ...
        self.SupplierTable["show"]="headings"    #Show Only Our Headings
    ...
    def update(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error", "Invalid Invoice No., try diffrent",parent=self.root)
